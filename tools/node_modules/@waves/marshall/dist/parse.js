...
exports.parseHeader = exports.parserFromSchema = void 0;
 ...
 */
const parserFromSchema = (schema, toLongConverter) => (bytes, start = 0) => {
    let cursor = start;
    if (schema.type === 'array') {
        const result = [];
        const { value: len, shift } = (schema.fromBytes || parsePrimitives_1.P_SHORT)(bytes, start);
        cursor += shift;
        (0, utils_1.range)(0, len).forEach(_ => {
            const parser = (0, exports.parserFromSchema)(schema.items, toLongConverter);
        ...
        schema.schema.forEach(field => {
            ...
            const parser = (0, exports.parserFromSchema)(schema, toLongConverter);
            const { value, shift } = parser(bytes, cursor);
            cursor += shift;
            if (value !== undefined) {
        ...
        const parser = (0, exports.parserFromSchema)(item.schema, toLongConverter);
        const { value, shift } = parser(bytes, cursor);
        cursor += shift;
        return {
        ...
        const key = (0, parsePrimitives_1.byteToStringWithLength)(bytes, cursor);
        cursor += key.shift;
        let dataType = (0, parsePrimitives_1.P_BYTE)(bytes, cursor);
        ...
        const parser = (0, exports.parserFromSchema)(itemRecord[1], toLongConverter);
...
exports.parserFromSchema = parserFromSchema;
const parseHeader = (bytes) => {
    let shift = 0;
    let typeInfo = (0, parsePrimitives_1.P_BYTE)(bytes, shift);
    shift += typeInfo.shift;
    // ExchangeTransactionV2 have leading 0 in bodybytes
    if (typeInfo.value === 0) {
        typeInfo = (0, parsePrimitives_1.P_BYTE)(bytes, shift);
        shift += typeInfo.shift;
    }
    let versionInfo = (0, parsePrimitives_1.P_BYTE)(bytes, shift);
    return {
        type: typeInfo.value,
...
function parseTx(bytes, toLongConverter) {
    const { type, version } = (0, exports.parseHeader)(bytes);
    const schema = (0, schemas_1.getTransactionSchema)(type, version);
    return (0, exports.parserFromSchema)(schema, toLongConverter)(bytes).value;
}
/**
 * This function cannot parse OrderV1, which doesn't have version field
 */
function parseOrder(bytes, toLongConverter) {
    return (0, exports.parserFromSchema)(schemas_1.orderSchemaV2, toLongConverter)(bytes).value;
}
