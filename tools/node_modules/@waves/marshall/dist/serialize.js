...
exports.serializerFromSchema = void 0;
...
const serializerFromSchema = (schema, fromLongConverter) => (obj) => {
    //let result = Uint8Array.from([]);
    let serializer, itemBytes;
    if (schema.type === 'array') {
        serializer = (0, exports.serializerFromSchema)(schema.items, fromLongConverter);
        itemBytes = (0, utils_1.concat)(...obj.map((item) => serializer((item))));
        return (0, utils_1.concat)((schema.toBytes || serializePrimitives_1.SHORT)(obj.length), itemBytes);
    }
        ...
        schema.schema.forEach(field => {
            ...
            if (Array.isArray(name)) {
                data = name.reduce((acc, fieldName) => (Object.assign(Object.assign({}, acc), { [fieldName]: obj[fieldName] })), {});
            }
            else {
                data = obj[name];
            }
            serializer = (0, exports.serializerFromSchema)(schema, fromLongConverter);
            itemBytes = serializer(data);
            objBytes = (0, utils_1.concat)(objBytes, itemBytes);
        });
        if (schema.withLength) {
            const l = schema.withLength.toBytes(objBytes.length);
            objBytes = (0, utils_1.concat)(l, objBytes);
        }
        if (schema.optional)
            objBytes = (0, utils_1.concat)([1], objBytes);
        return objBytes;
    }
        ...
        if (anyOfItem.strKey === 'boolean' && anyOfItem.key === 6 && obj.value === false)
            anyOfItem.key = 7;
        // HACK END
        serializer = (0, exports.serializerFromSchema)(anyOfItem.schema, fromLongConverter);
        ...
        if (schema.valueField == null) {
            ...
            return (0, utils_1.concat)((schema.toBytes || serializePrimitives_1.BYTE)(anyOfItem.key), itemBytes);
        }
    }
        ...
        const keyBytes = (0, serializePrimitives_1.LEN)(serializePrimitives_1.SHORT)(serializePrimitives_1.STRING)(obj.key);
        const type = obj.type;
        ...
        serializer = (0, exports.serializerFromSchema)(typeSchema, fromLongConverter);
        itemBytes = serializer(obj.value);
        return (0, utils_1.concat)(keyBytes, (0, serializePrimitives_1.BYTE)(typeCode), itemBytes);
    }
    else {
        throw new Error(`Serializer Error: Unknown schema type: ${schema.type}`);
    }
};
exports.serializerFromSchema = serializerFromSchema;
function serializeTx(tx, fromLongConverter) {
    const { type, version } = tx;
    const schema = (0, schemas_1.getTransactionSchema)(type, version);
    return (0, exports.serializerFromSchema)(schema, fromLongConverter)(tx);
}
function serializeOrder(ord, fromLongConverter) {
    ...
    return (0, exports.serializerFromSchema)(schema, fromLongConverter)(ord);
}
