"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.massTransfer = massTransfer;
/**
 * @module index
 */
const ts_lib_crypto_1 = require("@waves/ts-lib-crypto");
const marshall_1 = require("@waves/marshall");
const ts_types_1 = require("@waves/ts-types");
const generic_1 = require("../generic");
const validators_1 = require("../validators");
const proto_serialize_1 = require("../proto-serialize");
const defaultVersions_1 = require("../defaultVersions");
function massTransfer(paramsOrTx, seed) {
    const type = ts_types_1.TRANSACTION_TYPE.MASS_TRANSFER;
    const version = paramsOrTx.version || defaultVersions_1.DEFAULT_VERSIONS.MASS_TRANSFER;
    const seedsAndIndexes = (0, generic_1.convertToPairs)(seed);
    const senderPublicKey = (0, generic_1.getSenderPublicKey)(seedsAndIndexes, paramsOrTx);
    if (!Array.isArray(paramsOrTx.transfers) || paramsOrTx.transfers.length === 0)
        throw new Error('Should contain at least one transfer');
    const tx = {
        type,
        version,
        senderPublicKey,
        assetId: (0, generic_1.normalizeAssetId)(paramsOrTx.assetId),
        transfers: paramsOrTx.transfers,
        fee: (0, generic_1.fee)(paramsOrTx, 100000 + Math.ceil(0.5 * paramsOrTx.transfers.length) * 100000),
        timestamp: paramsOrTx.timestamp || Date.now(),
        attachment: paramsOrTx.attachment || '',
        proofs: paramsOrTx.proofs || [],
        chainId: (0, generic_1.networkByte)(paramsOrTx.chainId, (0, generic_1.chainIdFromRecipient)(paramsOrTx.transfers[0]?.recipient)),
        id: '',
    };
    validators_1.validate.massTransfer(tx);
    const bytes = version > 1 ? (0, proto_serialize_1.txToProtoBytes)(tx) : marshall_1.binary.serializeTx(tx);
    seedsAndIndexes.forEach(([s, i]) => (0, generic_1.addProof)(tx, (0, ts_lib_crypto_1.signBytes)(s, bytes), i));
    tx.id = (0, ts_lib_crypto_1.base58Encode)((0, ts_lib_crypto_1.blake2b)(bytes));
    return tx;
}
//# sourceMappingURL=mass-transfer.js.map