"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.setScript = setScript;
/**
 * @module index
 */
const ts_lib_crypto_1 = require("@waves/ts-lib-crypto");
const marshall_1 = require("@waves/marshall");
const ts_types_1 = require("@waves/ts-types");
const generic_1 = require("../generic");
const validators_1 = require("../validators");
const proto_serialize_1 = require("../proto-serialize");
const defaultVersions_1 = require("../defaultVersions");
function setScript(paramsOrTx, seed) {
    const type = ts_types_1.TRANSACTION_TYPE.SET_SCRIPT;
    const version = paramsOrTx.version || defaultVersions_1.DEFAULT_VERSIONS.SET_SCRIPT;
    const seedsAndIndexes = (0, generic_1.convertToPairs)(seed);
    const senderPublicKey = (0, generic_1.getSenderPublicKey)(seedsAndIndexes, paramsOrTx);
    if (paramsOrTx.script === undefined)
        throw new Error('Script field cannot be undefined. Use null explicitly to remove script');
    let scriptBytes = (0, proto_serialize_1.scriptToProto)(paramsOrTx.script);
    const computedFee = scriptBytes != null ? (Math.ceil(scriptBytes.length / 1024) * 100000) : 500000;
    const tx = {
        type,
        version,
        senderPublicKey,
        chainId: (0, generic_1.networkByte)(paramsOrTx.chainId, 87),
        fee: (0, generic_1.fee)(paramsOrTx, computedFee),
        timestamp: paramsOrTx.timestamp || Date.now(),
        proofs: paramsOrTx.proofs || [],
        id: '',
        script: (0, generic_1.base64Prefix)(paramsOrTx.script),
    };
    validators_1.validate.setScript(tx);
    const bytes = version > 1 ? (0, proto_serialize_1.txToProtoBytes)(tx) : marshall_1.binary.serializeTx(tx);
    seedsAndIndexes.forEach(([s, i]) => (0, generic_1.addProof)(tx, (0, ts_lib_crypto_1.signBytes)(s, bytes), i));
    tx.id = (0, ts_lib_crypto_1.base58Encode)((0, ts_lib_crypto_1.blake2b)(bytes));
    return tx;
}
//# sourceMappingURL=set-script.js.map