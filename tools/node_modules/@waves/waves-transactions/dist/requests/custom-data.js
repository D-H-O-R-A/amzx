...
function customData(cData, seed) {
    ...
    const hash = (0, ts_lib_crypto_1.base58Encode)((0, ts_lib_crypto_1.blake2b)(bytes));
    const pk = cData.publicKey ? cData.publicKey : seed && (0, ts_lib_crypto_1.publicKey)(seed);
    const signature = seed && (0, ts_lib_crypto_1.signBytes)(seed, bytes);
    return { ...cData, hash, publicKey: pk, signature };
}
function serializeCustomData(d) {
    if (d.version === 1) {
        return (0, ts_lib_crypto_1.concat)([255, 255, 255, 1], marshall_1.serializePrimitives.BASE64_STRING(d.binary));
    }
    else if (d.version === 2) {
        const ser = marshall_2.binary.serializerFromSchema(marshall_1.schemas.txFields.data[1]);
        return (0, ts_lib_crypto_1.concat)([255, 255, 255, 2], ser(d.data));
    }
    else {
        //@ts-ignore
