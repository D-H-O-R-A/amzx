...
class WavesBlockchainApiGrpcService(context: ExtensionContext, allowedBlockchainStateAccounts: Set[ByteStr], lpAccounts: Set[ByteStr])(implicit
  ...
  override def getStatuses(request: TransactionsByIdRequest): Future[TransactionsStatusesResponse] = Future {
    val statuses = request.transactionIds.map { txId =>
      context.blockchain.transactionInfo(txId.toVanilla).map(_._1) match {
        case Some(TxMeta(height, _, _)) => TransactionStatus(txId, TransactionStatus.Status.CONFIRMED, height.toInt.toLong)
        case None =>
    ...
    val r = context.blockchain.assetScript(asset) match {
      ...
      case Some(info) =>
        ...
        parseScriptResult(
          ScriptRunner(
            in = tx,
            blockchain = SnapshotBlockchain(context.blockchain, utxState.get().getAccountsDiff(context.blockchain)),
            script = info.script,
            isAssetScript = true,
            scriptContainerAddress = Environment.AssetId(asset.byteRepr),
  ...
  override def runAddressScript(request: RunAddressScriptRequest): Future[RunScriptResponse] = Future {
    ...
    val r = context.blockchain.accountScript(address) match {
      ...
      case Some(scriptInfo) =>
        ...
        if (allowedBlockchainStateAccounts.contains(order.senderPublicKey.byteStr) || lpAccounts.contains(order.senderPublicKey.byteStr)) {
          val blockchain = SnapshotBlockchain(context.blockchain, utxState.get().getAccountsDiff(context.blockchain))
  ...
  override def getAddressFullRegularBalance(request: GetAddressFullRegularBalanceRequest): Future[GetAddressFullRegularBalanceResponse] = {
    ...
    for {
      ...
      assetBalances <- context.accountsApi.portfolio(address).toListL.map(_.flatten)
