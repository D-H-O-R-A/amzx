...
case class UtxState(
  ...
  transactions: Map[ByteStr, (UtxTransaction, Transaction, StateSnapshot)] = Map()
) extends ScorexLogging {

  def getAccountsDiff(blockchain: Blockchain): StateSnapshot = {
    def relevantDiff(diff: StateSnapshot): Boolean =
    ...
    transactions.foldLeft(StateSnapshot.empty) { case (acc, (_, (_, tx, d))) =>
  ...
  private def add(tx: Transaction, diff: StateSnapshot): (Option[UtxEvent], UtxState) = {
