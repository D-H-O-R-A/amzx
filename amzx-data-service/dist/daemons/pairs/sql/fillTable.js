...
const selectPairsCTE = pg
    ...
    qb.select({
        ...
        weighted_average_price: pg.raw('floor(sum(e.amount::numeric * e.price::numeric)/ sum(e.amount))'),
        volume_waves: pg.raw(`case when amount_asset_id='AMZX' then sum(e.amount) when price_asset_id='AMZX' then sum(e.amount::numeric * e.price::numeric) end`),
    ...
    .from({ p: 'pairs_cte' })
    .columns('p.amount_asset_id', 'p.price_asset_id', 'p.first_price', 'p.last_price', 'p.volume', {
...
}, 'p.quote_volume', 'p.high', 'p.low', 'p.weighted_average_price', 'p.txs_count', 'p.matcher_address')
    .leftJoin({ p1: 'pairs_cte' }, function () {
    this.on(pg.raw(`p1.amount_asset_id='AMZX'`))
    ...
    this.on('p2.amount_asset_id', 'p.price_asset_id')
        .andOn(pg.raw(`p2.price_asset_id='AMZX'`))
        .andOn('p2.matcher_address', 'p.matcher_address');
});
exports.fillTable = (pairsTableName) => pg.into(pairsTableName).insert(selectPairsCTE).toString();
