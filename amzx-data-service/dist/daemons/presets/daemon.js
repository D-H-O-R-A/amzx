...
const loop = (func, cfg, interval, timeout) => {
    ...
    return Task.waitAny([
        func(cfg),
        Task.task(resolver => {
            const timerId = setTimeout(() => resolver.reject(new Error('Daemon timeout expired')), timeout);
...
const main = (daemon, config, interval, timeout, logger) => Task.of(Maybe.fromNullable(daemon.init))
    .map(tap(maybeInit => maybeInit.matchWith({
    Nothing: () => logger.warn({
        message: '[DAEMON] init function not found',
    }),
    Just: () => { },
})))
    .chain(maybeInit => logTaskProgress(logger)({
    start: timeStart => ({
    ...
    .map(tap(maybeLoop => maybeLoop.matchWith({
    Nothing: () => logger.warn({
        message: '[DAEMON] loop function not found',
    }),
    Just: () => { },
})))
    .chain(maybeLoop => logTaskProgress(logger)({
    start: timeStart => ({
    ...
    Just: ({ value }) => Task.task(resolver => loop(value, config, interval, timeout).catch(e => resolver.reject(e))),
