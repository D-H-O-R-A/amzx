...
describe('Pointfree methods should work with real knex object', () => {
    ...
    it('`orWhere` pointfree', () => {
        expect(qIncludes(['select', 't', 'x'], q)).toBe(true);
        const qWhereId2OrSenderGte3 = compose(orWhere('sender', '>=', 3), where('id', 2))(q);
        expect(qIncludes(['where', 'id', '2', 'or', 'sender', '>=', '3'], qWhereId2OrSenderGte3)).toBe(true);
    });
});
//# sourceMappingURL=knex.real.test.js.map