...
const matchRequestsResults = curryN(3, (matchFn, requests, results) => {
    const findResult = compose(Maybe.fromNullable, req => find(res => matchFn(req, res), results));
 ...
 *    filter[]
 * */
const pickBindFilters = curryN(3, (F, fsToApply, fValues) => compose(map(x => F[x](fValues[x])), filter(has(__, fValues)))(fsToApply));
/**
 ...
 * @returns {string}
 */
const prepareForLike = (query, params = { matchExactly: false }) => compose(query => (params.matchExactly ? query : `${query}%`), escapeForLike)(query);
module.exports = {
    matchRequestsResults,
    pickBindFilters,
    escapeForTsQuery,
