...
const { leftNotNullMonoid, rightNotNullMonoid, sumMonoid, bigNumberPlusMonoid, maxMonoid, bigNumberMaxMonoid, bigNumberMinMonoid, weightedAveragePriceMonoid, candleMonoid, } = require('../candleMonoid');
...
describe('monoid', () => {
    ...
    describe('big number plus', () => {
        it('shourd return big number of sum of args', () => {
            expect(bigNumberPlusMonoid.concat(BigNumber(1), BigNumber(2))).toEqual(BigNumber(3));
        });
    });
    describe('max', () => {
    ...
    describe('big number max', () => {
        it('should return bignumber max monoid', () => {
            expect(bigNumberMaxMonoid.concat(BigNumber(10), BigNumber(2))).toEqual(BigNumber(10));
        });
    });
    describe('big number min', () => {
        it('should return bignumber min monoid', () => {
            expect(bigNumberMinMonoid.concat(BigNumber(10), BigNumber(2))).toEqual(BigNumber(2));
        });
    });
    describe('weighted average price', () => {
        it('should return weighted average price of candles monoid', () => {
            expect(weightedAveragePriceMonoid.concat({ quote_volume: BigNumber(2), volume: BigNumber(3) }, { quote_volume: BigNumber(3), volume: BigNumber(2) })).toEqual(BigNumber(1));
        });
    });
    describe('candle', () => {
        it('should return two concated candle monoids', () => {
            ...
            expect(monoid.weighted_average_price.comparedTo(BigNumber('0.00116116966924700914'))).toEqual(0);
        });
    });
});
//# sourceMappingURL=candleMonoid.test.js.map