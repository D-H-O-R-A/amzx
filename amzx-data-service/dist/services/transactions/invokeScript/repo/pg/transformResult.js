...
const { always, complement, compose, cond, either, filter, groupBy, isEmpty, isNil, map, omit, prop, sortBy, T, uniqWith, values, } = require('ramda');
const isNotNil = complement(isNil);
const getArgFieldByType = (type) => {
    switch (type) {
        case 'integer':
...
const buildTxFromTxs = (txs) => {
    ...
    // fill tx.payment
    tx.payment = compose(map(omit(['positionInPayment'])), sortBy(prop('positionInPayment')), uniqWith((a, b) => a.positionInPayment === b.positionInPayment), map(getPaymentItem), filter((tx) => isNotNil(prop('amount', tx))))(txs);
    // fill tx.call
    if (!isNil(firstRaw.function_name)) {
        tx.call = {
            function: firstRaw.function_name,
            args: compose(map(omit(['positionInArgs'])), sortBy(prop('positionInArgs')), uniqWith((a, b) => a.positionInArgs === b.positionInArgs), map(getArg), filter((tx) => isNotNil(prop('arg_type', tx))))(txs),
        };
    }
    return tx;
};
