"use strict";
const { compose, groupBy, prop, values, reduce, map, assoc, append, omit, merge, identity, isNil, ifElse, cond, always, isEmpty, either, sortBy, evolve, T, } = require('ramda');
const getDataObject = (txRow) => ({
    key: txRow.data_key,
    type: txRow.data_type,
    value: txRow[`data_value_${txRow.data_type}`],
    positionInTx: txRow.position_in_tx,
});
...
const appendRowToTx = (tx, row) => compose(ifElse(() => isNil(prop('data_key', row)), identity, assoc('data', append(getDataObject(row), tx.data))), merge(removeDataEntryFromRow(row)))(tx);
/**
...
const dataEntriesToTxs = cond([
    ...
    [
        ...
        })), map(reduce(appendRowToTx, { data: [] })), values, groupBy(prop('id'))),
    ],
]);
