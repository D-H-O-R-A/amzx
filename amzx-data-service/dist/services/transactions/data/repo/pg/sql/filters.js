...
const byKey = curryN(2, (key, q) => q.clone().whereIn('t.uid', function () {
    this.select('tx_uid').from('txs_12_data').where('data_key', key);
}));
const byType = curryN(2, (type, q) => q.clone().whereIn('t.uid', function () {
...
module.exports = Object.assign(Object.assign({}, commonFilters), { key: byKey, type: byType, value: byValue, timeStart: byTimeStamp('>='), timeEnd: byTimeStamp('<='), blockTimeStart: byBlockTimeStamp('>='), blockTimeEnd: byBlockTimeStamp('<=') });
