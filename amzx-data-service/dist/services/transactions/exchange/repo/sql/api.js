...
const createApi = ({ filters: F }) => ({
    ...
    F.limit(1), selectFromFiltered, String)(select),
    mget: (ids) => pipe(F.ids(ids), 
    // tip for postgresql to use index
    F.sort(defaultValues.SORT), F.limit(ids.length), selectFromFiltered, String)(select),
    search: (fValues) => {
        const fNames = [
            // tx attributes
            'timeStart',
        ...
        return pipe(...fs, selectFromFiltered, F.limit(fValues.limit), F.sort(sort), String)(select);
    },
});
