"use strict";
const { where, whereIn, whereRaw, limit } = require('../../../../utils/db/knex/index');
...
const byRecipient = (addressOrAlias) => whereRaw(`recipient_address = coalesce((select sender from txs_10 where alias = '${addressOrAlias}' limit 1), '${addressOrAlias}')`);
const byScript = (s) => whereRaw('md5(script) = ?', md5(s));
const sort = (s) => (q) => q.clone().orderBy('t.uid', s);
const after = ({ uid, sort }) => (q) => q.clone().whereRaw(`t.uid ${sort === 'desc' ? '<' : '>'} ${uid.toString()}`);
module.exports = {
    id,
    ids,
    sender,
